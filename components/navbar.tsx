  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        scrolled ? "backdrop-panel" : "bg-transparent"
      }`}
    >
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="md:hidden">
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="backdrop-panel border-primary/20 w-[280px] sm:w-[350px]">
                <div className="flex flex-col space-y-6 mt-8">
                  {navItems.map((item) => (
                    <Link
                      key={item.name}
                      href={item.href}
                      className="text-lg text-gray-300 hover:text-primary transition-colors duration-200 py-2"
